...
public class NbtIOShim {
    ...
    public static <T> CompoundTag encode(Codec<T> codec, T value) {
        //? if <1.20.5
        //return (CompoundTag) codec.encodeStart(NbtOps.INSTANCE, value).getOrThrow(false, s -> { throw new RuntimeException(s); });
        //? if >=1.20.5
    ...
    public static <T> T decode(Codec<T> codec, CompoundTag tag) {
        //? if <1.20.5
        //return codec.parse(NbtOps.INSTANCE, tag).getOrThrow(false, s -> { throw new RuntimeException(s); });
        //? if >=1.20.5
