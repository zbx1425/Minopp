...
public class PlayBadgeGuiShard extends BadgeGuiShard {
    ...
    public void render(GuiGraphicsExtractor g, Font font, int x, int y, int tintColor, int alpha) {
        ...
        //? if <26.1 {
        /*g.blit(x + ((card.suit == Card.Suit.WILD && card.number == -1) ? 16 : 1),
            y, 0, 16, 16, ShardResources.INSTANCE.getSpriteForSuit(card.getEquivSuit()));
        *///? } else {
        ...
        if (card.family == Card.Family.REVERSE) {
            ...
            GuiShim.drawString(g, font, briefNumberRepr, x + 16 + (16 - reprWidth) / 2, y + (16 - 8) / 2, alphaWhite);
        }
    }
}
